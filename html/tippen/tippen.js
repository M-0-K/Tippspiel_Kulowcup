  ...
  // Issue #23: Dynamische Punkte-Tabelle für die Gruppenphase laden
function ladeGruppenTabellen() {
    $.get("../spiele_backend.php", { action: "getGroupTable" }, function(data) {
        let container = document.getElementById("gruppen-tabellen-container");
        if (!container || !data.groups) return;

        let html = "";
        
        // Jede Gruppe (A, B, C, D) durchlaufen
        data.groups.forEach(function(gruppe) {
            html += `<div class="group-table-wrapper" style="margin-bottom: 30px;">`;
            html += `<h2 class="punktestand-title">Tabelle Gruppe ${gruppe.phase}</h2>`;
            html += `<div class="group-table">`;
            
            // Tabellen-Kopf (Nutzt seine eigenen Klassen)
            html += `<div class="row header flex" style="margin-bottom: 5px;">
                        <div class="cell" style="flex: 2; text-align: left;">Team</div>
                        <div class="cell">Sp</div>
                        <div class="cell">S</div>
                        <div class="cell">U</div>
                        <div class="cell">N</div>
                        <div class="cell">Tore</div>
                        <div class="cell">Diff</div>
                        <div class="cell" style="color: gold;">Pkt</div>
                     </div>`;

            // Jedes Team in der Gruppe durchlaufen
            gruppe.teams.forEach(function(team) {
                html += `<div class="table row flex clickablerow">
                    <div class="cell team-cell" style="flex: 2; font-weight: bold;">
                        <span class="team-name">${team.name}</span>
                    </div>
                    <div class="cell">${team.spiele}</div>
                    <div class="cell">${team.siege}</div>
                    <div class="cell">${team.unentschieden}</div>
                    <div class="cell">${team.niederlagen}</div>
                    <div class="cell">${team.tore}:${team.gegentore}</div>
                    <div class="cell">${team.differenz}</div>
                    <div class="cell score">${team.punkte}</div>
                </div>`;
            });
...
// Funktion starten, sobald die Seite lädt
ladeGruppenTabellen();
